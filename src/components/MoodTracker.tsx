    ...
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      ...
      {entries.length > 0 && (
        <div className="grid grid-cols-2 gap-4">
          <div className="card text-center">
            <div className="text-3xl font-semibold font-display text-teal">{entries.length}</div>
            <div className="text-sm text-ink-soft mt-1">总记录数</div>
          </div>
          <div className="card text-center">
            <div className="text-3xl font-semibold font-display text-teal-deep">{avgMood}</div>
            <div className="text-sm text-ink-soft mt-1">平均情绪</div>
          </div>
        </div>
      )}

      {weekInsight && (
        <div className="card bg-gradient-to-br from-teal-soft/70 to-sand/40 border-teal/20">
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 rounded-xl bg-teal-soft flex items-center justify-center shrink-0">
              <Sparkles className="w-4 h-4 text-teal" />
            </div>
            <div className="flex-1 min-w-0">
              <h2 className="font-medium text-ink text-sm">本周洞察</h2>
              <p className="text-sm text-ink-soft mt-1 leading-relaxed">{weekInsight}</p>
              <div className="flex flex-wrap gap-2 mt-3">
                <Link href="/practice" className="text-xs text-teal hover:underline">
                  去做正念练习 →
                </Link>
                <Link href="/journal" className="text-xs text-teal hover:underline">
                  写情绪日记 →
                </Link>
              </div>
            </div>
      ...
      {showForm && (
        <div className="card animate-fade-in-up">
          <h2 className="font-medium text-ink mb-4">此刻的感受</h2>
            ...
            {[1, 2, 3, 4, 5].map((mood) => (
              <button
                ...
                className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-xl transition-all ${
                  selectedMood === mood
                    ? "bg-teal-soft border-2 border-teal scale-105"
                    : "bg-white/45 border-2 border-transparent hover:bg-mist/60"
                }`}
              >
                <span className="text-2xl">{MOOD_EMOJIS[mood - 1]}</span>
                <span className="text-xs text-ink-soft">{MOOD_LABELS[mood - 1]}</span>
              </button>
            ))}
          </div>

          <div className="mb-4">
            <label className="text-sm text-ink-soft mb-2 block">标签（可选）</label>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm transition-all ${
                    selectedTags.includes(tag)
                      ...
                      : "bg-mist/60 text-ink-soft hover:bg-gray-200"
                  }`}
                >
                  {tag}
                </button>
      ...
      <div className="space-y-3">
          ...
          <div key={entry.id} className="card flex items-start gap-4 animate-fade-in-up">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-medium text-ink">{MOOD_LABELS[entry.mood - 1]}</span>
                <span className="text-xs text-ink-soft/70">
                  {formatDateTime(entry.timestamp)}
                </span>
              </div>
              {entry.note && <p className="text-sm text-ink-soft mt-1">{entry.note}</p>}
              {entry.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {entry.tags.map((tag) => (
                    <span key={tag} className="text-xs bg-mist/60 text-ink-soft px-2 py-0.5 rounded-md">
                      {tag}
                    </span>
                  ))}
                </div>
        ...
        {entries.length === 0 && !showForm && (
          <div className="card text-center py-12">
            <p className="text-ink-soft/70">还没有情绪记录</p>
            <p className="text-sm text-ink-soft/70 mt-1">开始记录，了解自己的情绪模式</p>
          </div>
        )}
      </div>
    </div>
